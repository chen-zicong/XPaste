...
- 单列轻量主面板，雾蓝选中态与固定行高；底部提供快捷键提示、复制和更多操作
- 按空格、`⌘I` 或预览按钮打开独立悬浮预览；主面板尺寸、选择与滚动位置保持稳定
- 预览默认只读，支持选择和复制文字；点击“编辑”进入编辑器，关闭或切换条目会保留未保存草稿
- 明细采用中性灰白阅读布局与系统字体，复制与粘贴位于底部；浅深色外观、增强对比度与减少动态效果均有对应处理
